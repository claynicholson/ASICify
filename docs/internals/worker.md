...
Python 3.11+ process that takes a model and produces synthesizable
Verilog plus a bit-exact reference plus a hardware estimate. The same
code runs as a CLI for local use without the hosted service.
...
│   │   ├── parse.py       torch.fx-style module walk + HF attention detection
│   │   ├── sparsity.py    Wraps kernels.sparsity, runs before quantize
│   │   ├── decompose.py   SVD / Monarch / butterfly factorization
...
apply_decomposition(graph, config)
   ↓ ModelGraph with factored layers (SVD / Monarch / butterfly)
...
through, and decomposition also runs before quantization so the factored
layers are what get quantized.
...
- `tests/test_quantize_multi.py`: every precision × generated reference,
  bit-exact
- `tests/test_end_to_end.py`: 32-trial random check on full pipeline
- `tests/test_simulation.py`: generates a package and runs its
  `make sim` (Verilator + cocotb) when the tools are installed

Change `kernels/quantize.py` or `kernels/pack.py` or
`rtl/templates/linear_layer.v.j2` or `rtl/templates/reference.py.j2`,
then run those tests in the same commit. The fast ones take seconds.
...
3. `worker/pipeline/parse.py`: extending HF attention pattern coverage
...
a problem that should be solved in one of those five places instead.
